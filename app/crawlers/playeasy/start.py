...
from app.crawlers.playeasy.get import crawl
...
def main():
    client = HttpClient(base_url='https://www.playeasy.com.br')
    for page in range(args.start, args.stop + 1):
        try:
            raw_response = {'response': str(crawl(client, page))}
        ...
        if inbox_state == State.PROCESSING:
            process_inbox(new_inbox_entry.id, 'app.crawlers.playeasy.processor')
